        ...
        <main class="tm-main">
			...
			<?php
				if (!isset($_COOKIE['role'])){
					echo "<p style='color:red'>You are not logged in!";
					exit();
				}
				if(isset($_COOKIE['role']) && $_COOKIE['role']!="ADMIN"){
					echo "<p style='color:red'>You are not an admin!!";
					exit();
				}
				$host="localhost";
				$user="root";
				...
				$result = $con->query("SELECT ID, Email, Login, Password, Active FROM user");
				if ($result->num_rows > 0) {
				  // output data of each row
				  ?>
				  <table id="users" style="collapse-border:collapse; border: 1px solid black"><th>ID</th><th>Email</th><th>Login</th><th>Active</th><th>Edit</th>
				  <?php
				  while($row = $result->fetch_assoc()) {
					...
					$row["Email"]."</td>". "<td  style='collapse-border:collapse; border: 1px solid black'>" . 
					$row["Login"]."</td>" . "<td  style='collapse-border:collapse; border: 1px solid black'>".
					$row["Active"]."</td>" . "<td  style='collapse-border:collapse; border: 1px solid black'>".
					"<form method='POST' action='admin-edit.php?id=".$row["ID"]."'><input type='submit'/></form>";
				  }
				} else {
				  echo "0 results";
				}
